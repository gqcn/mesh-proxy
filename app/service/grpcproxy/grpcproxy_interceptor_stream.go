...
// GRPC中间件 - Stream
func gRPCStreamInterceptor(req interface{}, stream grpc.ServerStream, info *grpc.StreamServerInfo, handler grpc.StreamHandler) (err error) {
	var (
		array          []string
		host           string
		agent          string
	...
	incomingHeaders, _ := metadata.FromIncomingContext(stream.Context())
